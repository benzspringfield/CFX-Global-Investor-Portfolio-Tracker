"""
ทะเบียนนักลงทุน + การตั้งค่ากลางของระบบ
แก้ไฟล์นี้ไฟล์เดียวเพื่อเพิ่ม/ลบนักลงทุนในอนาคต
"""
import os
from pathlib import Path

# ---------------------------------------------------------------------------
# พาธพื้นฐาน
# ---------------------------------------------------------------------------
ROOT = Path(__file__).resolve().parent
DATA_DIR = ROOT / "data"
DATA_DIR.mkdir(exist_ok=True)
DB_PATH = DATA_DIR / "portfolios.db"


def _secret(key, default=""):
    """
    อ่านค่าลับตามลำดับ: env var -> st.secrets (เฉพาะตอนรันใน Streamlit) -> default
    ทำให้ใช้ได้ทั้งใน GitHub Action (env) และบน Streamlit Cloud (secrets)
    โดยไม่ commit ค่าลับลง repo
    """
    v = os.environ.get(key)
    if v:
        return v
    try:
        import streamlit as st
        if key in st.secrets:
            return st.secrets[key]
    except Exception:
        pass
    return default


# SEC บังคับให้ใส่ User-Agent ที่ระบุตัวตน (ชื่อ + อีเมล) ไม่งั้นโดน 403
SEC_USER_AGENT = _secret("SEC_USER_AGENT", "Portfolios-Tracker benzspringfield@gmail.com")
HTTP_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"

# OpenFIGI API key (ฟรี สมัครที่ openfigi.com) — ใช้แปลง CUSIP -> ticker
OPENFIGI_API_KEY = _secret("OPENFIGI_API_KEY", "")

# รหัสผ่านเข้าแอป (ตั้งใน st.secrets บน Streamlit Cloud) — เว้นว่าง = ไม่ล็อก (รันในเครื่อง)
APP_PASSWORD = _secret("APP_PASSWORD", "")

# Anthropic API key สำหรับปุ่ม "วิเคราะห์เชิงลึกด้วย AI" — เว้นว่าง = ซ่อนปุ่ม (ใช้แต่สูตรฟรี)
ANTHROPIC_API_KEY = _secret("ANTHROPIC_API_KEY", "")

# ---------------------------------------------------------------------------
# ทะเบียนนักลงทุน
# source:
#   "edgar_13f"  -> ดึงจาก SEC 13F (ต้องมี cik)
#   "ark_daily"  -> ดึง holdings รายวันจาก arkfunds.io (ต้องมี ark_symbols)
#   "congress"   -> ดึงจาก House/Senate disclosure (ต้องมี congress_name)
#   "methodology"-> ไม่มี filing สาธารณะ ใช้ระบบสัญญาณแทน (skill)
# ---------------------------------------------------------------------------
INVESTORS = {
    "druckenmiller": {
        "display": "Stanley Druckenmiller (Duquesne Family Office)",
        "source": "edgar_13f",
        "cik": "0001536411",
        "style": "Macro / concentrated growth",
        "notes": "เห็นเฉพาะ long หุ้น US รายไตรมาส ดีเลย์ ~45 วัน ไม่เห็น short/options",
        "skill": "druckenmiller-trader",
    },
    "situational_awareness": {
        "display": "Leopold Aschenbrenner (Situational Awareness LP)",
        "source": "edgar_13f",
        "cik": "0002045724",
        "style": "AGI / AI thesis — concentrated",
        "notes": "กองใหม่ (เปิด 2024) เน้นธีม AI/AGI มี 13F ย้อนหลังตั้งแต่ Q2/2025",
        "skill": None,
    },
    "two_sigma": {
        "display": "Two Sigma Investments",
        "source": "edgar_13f",
        "cik": "0001179392",
        "style": "Quant / systematic (พอร์ตกระจายมาก)",
        "notes": "3,000+ position — 13F เป็น noise ใช้ดูธีมรวมได้ ไม่เหมาะถอดจังหวะ",
        "skill": None,
    },
    "ark": {
        "display": "ARK Invest (Cathie Wood)",
        "source": "ark_daily",
        "cik": "0001697748",                 # ยังดึง 13F สำรองได้
        "ark_symbols": ["ARKK", "ARKW", "ARKG", "ARKQ", "ARKF", "ARKX"],
        "style": "High-growth / disruptive innovation",
        "notes": "เปิด holdings รายวัน — ข้อมูลละเอียดที่สุดในกลุ่มนี้",
        "skill": None,
    },
    # --- เพิ่ม 30 ส.ค. 2026 (คัดกรองด้วย scratchpad/screen_candidates.py: จำลองพอร์ต 13F top-15 vs SPY, พ.ค.2025→ส.ค.2026) ---
    "baupost": {
        "display": "Seth Klarman (Baupost Group)",
        "source": "edgar_13f",
        "cik": "0001061768",
        "style": "Value / contrarian — concentrated",
        "notes": "พอร์ต 13F จำลอง +51% vs SPY +32% (alpha CAGR +13, MDD ต่ำ) สไตล์ value ถ่วงดุลกลุ่ม AI/growth ในแอป",
        "skill": None,
    },
    "coatue": {
        "display": "Philippe Laffont (Coatue Management)",
        "source": "edgar_13f",
        "cik": "0001135730",
        "style": "Tech / AI crossover — long/short",
        "notes": "พอร์ต 13F จำลอง +38% vs SPY +32%; สื่อรายงาน 2026 YTD +14.3% ถึง ก.ค. (Bloomberg) ทนตลาด AI ร่วงได้ดีกว่ากลุ่ม",
        "skill": None,
    },
    "viking": {
        "display": "Andreas Halvorsen (Viking Global Investors)",
        "source": "edgar_13f",
        "cik": "0001103804",
        "style": "Fundamental long/short — hedge หนัก",
        "notes": "พอร์ต 13F จำลอง +38% vs SPY +32% MDD ต่ำสุดในกลุ่ม; ผลจริงที่รายงานต่ำ (+2.5% YTD ก.ค. 2026) เพราะ short — ใช้เฉพาะสัญญาณฝั่ง long",
        "skill": None,
    },
    # --- เพิ่ม 30 ส.ค. 2026 ชุดที่ 2 (คุณเบ้นเลือกเอง — ชาลท้วงว่าจะทำให้ Consensus เอียงไปธีม AI) ---
    "whale_rock": {
        "display": "Alex Sacerdote (Whale Rock Capital)",
        "source": "edgar_13f",
        "cik": "0001387322",
        "style": "Tech / AI long/short — ผันผวนสูง",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +36% alpha +2 MDD -12.6%; สื่อรายงาน 2026: +72.5% H1 แล้ว -21.7% ก.ค. (Bloomberg)",
        "skill": None,
    },
    "lone_pine": {
        "display": "Stephen Mandel (Lone Pine Capital)",
        "source": "edgar_13f",
        "cik": "0001061165",
        "style": "Growth long/short (Tiger cub)",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +8% alpha -18 — กำไรจริงมาจาก short/จังหวะ ไม่ใช่ฝั่ง long; สื่อรายงาน +43% H1 2026 (Bloomberg)",
        "skill": None,
    },
    "altimeter": {
        "display": "Brad Gerstner (Altimeter Capital)",
        "source": "edgar_13f",
        "cik": "0001541617",
        "style": "Tech / cloud / AI — concentrated",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +20% alpha -9 MDD -11.7%; สื่อรายงาน +34% YTD ก.ค. 2026 (ก.ค. -11%)",
        "skill": None,
    },
    "appaloosa": {
        "display": "David Tepper (Appaloosa)",
        "source": "edgar_13f",
        "cik": "0001656456",
        "style": "Macro / distressed / opportunistic",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +33% alpha +1; สื่อรายงาน 2025 +23.3%, H1 2026 +32% gross (Bloomberg) — กำไรจากจังหวะที่ 13F ไม่เห็น",
        "skill": None,
    },
    "d1": {
        "display": "Daniel Sundheim (D1 Capital Partners)",
        "source": "edgar_13f",
        "cik": "0001747057",
        "style": "Public/private crossover long/short",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +36% alpha +3; สื่อรายงาน 2025 +33.7% (II Rich List)",
        "skill": None,
    },
    "tiger_global": {
        "display": "Chase Coleman (Tiger Global Management)",
        "source": "edgar_13f",
        "cik": "0001167483",
        "style": "Tech growth long/short + crossover",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +16% alpha -12.5 MDD -11.4%; สื่อรายงาน 2025 +7.9%, YTD ก.ค. 2026 +9.4%",
        "skill": None,
    },
    "third_point": {
        "display": "Daniel Loeb (Third Point)",
        "source": "edgar_13f",
        "cik": "0001040273",
        "style": "Event-driven / activist",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +6% alpha -20 แพ้ SPY ทุกไตรมาส; สื่อรายงาน YTD ก.ค. 2026 +0.8% (Institutional Investor)",
        "skill": None,
    },
    "tci": {
        "display": "Chris Hohn (TCI Fund Management)",
        "source": "edgar_13f",
        "cik": "0001647251",
        "style": "Concentrated quality / long-term activist",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +24% alpha -6; สื่อรายงาน 2025 +27.8%, Q1 2026 -9.4% (Institutional Investor)",
        "skill": None,
    },
    "bridgewater": {
        "display": "Bridgewater Associates (Pure Alpha)",
        "source": "edgar_13f",
        "cik": "0001350694",
        "style": "Macro / systematic — พอร์ต 13F กว้างมาก",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +36% alpha +2; สื่อรายงาน Pure Alpha 2025 +33%, H1 2026 +8.1% (Reuters) — 13F อ่านความเชื่อมั่นยากเหมือน Two Sigma",
        "skill": None,
    },
    "berkshire": {
        "display": "Warren Buffett (Berkshire Hathaway)",
        "source": "edgar_13f",
        "cik": "0001067983",
        "style": "Value / quality — long-only, ถือยาว",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +29% alpha -3; รายงานประจำปี 2025: per-share +10.9% vs S&P 500 +17.9%",
        "skill": None,
    },
    "pershing": {
        "display": "Bill Ackman (Pershing Square)",
        "source": "edgar_13f",
        "cik": "0001336528",
        "style": "Concentrated activist — 8-12 ตัว",
        "notes": "ผลคัดกรอง 30 ส.ค. 2026 (พอร์ต 13F top-15 จำลอง พ.ค.2025→ส.ค.2026 vs SPY +32%): +11% alpha -11; PSH NAV 2025 +20.9%, YTD 25 ส.ค. 2026 -3.4% (pershingsquareholdings.com)",
        "skill": None,
    },
    "scion": {
        "display": "Michael Burry (Scion Asset Management) [defunct]",
        "source": "edgar_13f",
        "cik": "0001649339",
        "style": "Contrarian / short via puts",
        "notes": "ถอนทะเบียน SEC พ.ย. 2025 ไม่มี 13F ใหม่ — เก็บไว้แบบเดียวกับ Melvin (แก้ survivorship) ระวัง: 13F ของ Scion มี put options ปนเป็น long",
        "defunct": True,
        "skill": None,
    },
    # --- กองที่ "เลิกยื่น 13F" แล้ว (ใส่เพื่อแก้ survivorship bias ใน backtest) ---
    "melvin": {
        "display": "Melvin Capital (Gabe Plotkin) [defunct]",
        "source": "edgar_13f",
        "cik": "0001628110",
        "style": "Growth/short — เจ๊งจาก GME short squeeze 2021, ปิดกอง 2022",
        "notes": "ยื่น 13F 2014→2023 แล้วหยุด — เคส 'เงินใหญ่ที่เจ๊ง' สำหรับแก้ survivorship",
        "defunct": True,
        "skill": None,
    },
    "greenlight": {
        "display": "Greenlight Capital (David Einhorn)",
        "source": "edgar_13f",
        "cik": "0001079114",
        "style": "Value/short — ขาดทุนหนักช่วง 2015-2018",
        "notes": "value investor ดังที่เคยแพ้ตลาดยาว — กันอคติว่าเงินใหญ่ชนะเสมอ",
        "defunct": False,
        "skill": None,
    },
    "pelosi": {
        "display": "Nancy Pelosi (US House)",
        "source": "congress",
        "congress_chamber": "house",
        "congress_name": "Pelosi",
        "style": "Long-dated mega-cap tech options + stock",
        "notes": "Periodic Transaction Report ดีเลย์ได้ถึง 45 วัน บอกช่วงมูลค่า ไม่บอกจำนวนแน่นอน",
        "skill": None,
    },
    "minervini": {
        "display": "Mark Minervini (methodology only)",
        "source": "methodology",
        "style": "SEPA / VCP momentum",
        "notes": "เทรดเดอร์ส่วนตัว ไม่มี filing — ใช้ระบบสัญญาณ minervini-trader แทนพอร์ตจริง",
        "skill": "minervini-trader",
    },
}

# นักลงทุนที่ใช้ดึง holdings มาทำ consensus/model ได้จริง (มี holdings ตัวเลข)
HOLDINGS_SOURCES = [k for k, v in INVESTORS.items()
                    if v["source"] in ("edgar_13f", "ark_daily")]
